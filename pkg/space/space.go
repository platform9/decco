...
func (c *SpaceRuntime) createPrivateIngressController() error {
	...
	for _, epName := range c.Space.Spec.PrivateIngressControllerTcpEndpoints {
		endpoints = append(endpoints, appspec.EndpointSpec{
			...
			Port: 80,
			SniHostname: epName +
				c.Space.Spec.PrivateIngressControllerTcpHostnameSuffix +
				"." + hostName,
		})
	}
...
func (c *SpaceRuntime) deleteHttpCert() error {
	...
	secrApi := c.kubeApi.CoreV1().Secrets(c.Space.Namespace)
	return secrApi.Delete(c.Space.Spec.HttpCertSecretName, &metav1.DeleteOptions{})
}
...
func (c *SpaceRuntime) deleteTcpCertAndCa() error {
	...
	secrApi := c.kubeApi.CoreV1().Secrets(c.Space.Namespace)
	return secrApi.Delete(c.Space.Spec.TcpCertAndCaSecretName, &metav1.DeleteOptions{})
}
